...
def registro_BBDD(uuid,path_image,image_tags):
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            controller.write_log(f"Insertando datos en la tabla pictures {query}", level="debug")
            conn.execute(query)
            conn.commit()
            ...
            for x in image_tags:
                ...
                controller.write_log(f"Insertando datos en la tabla tags {query}", level="debug")
                conn.execute(query)
                conn.commit()
            
                
    except Exception as e:
            controller.write_log(f"Error en la funcion registro_BBDD: {e}", level="error")
